...
function get_image(song_id)
{
    fetchData('http://172.30.13.160:5001/song/' + song_id).then(data => {
        if (data) {
            const image_url = "url(" + data["image"] + ")";
        ...
        songContainer.addEventListener('click', () => {
            fetchData('http://172.30.13.160:5001/song/' + image_info.song_id).then(data => {
                if (data) {
                    const image_url = "url(" + data["image"] + ")";
                    document.documentElement.style.setProperty('--alarm-bg', image_url);
                    play_music(data["lyric"], data["artist"], data["song_name"], data["result"])
                } else {
                    console.log('未能获取到数据');
                }
            });
...
function play_music_index(){
    const toggleButton = document.getElementById('toggleButton');
    const toggleDiv = document.getElementById('toggleDiv');

    toggleButton.addEventListener('click', function() {
        if (toggleDiv.classList.contains('visible')) {
            toggleDiv.classList.remove('visible'); // 移除 visible 类
            toggleDiv.classList.add('toggleDivB'); // 添加 hidden 类
        } else {
            toggleDiv.classList.remove('toggleDivB'); // 移除 hidden 类
            toggleDiv.classList.add('visible'); // 添加 visible 类
        }
    });
...
function sendRequest(send_data){
    ...
    fetch('http://172.30.13.160:5001/v3/chat-messages', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        const exisLyric = data.hasOwnProperty("lyric");
        const exisResult = data.hasOwnProperty("result");
        const exisArtist = data.hasOwnProperty("artist");
        const exisSongName = data.hasOwnProperty("song_name");
        const exisImage = data.hasOwnProperty("image");
        const songList= data.hasOwnProperty("play_list");

        if (exisSongName && exisArtist && exisResult)
        {
            if (exisImage)
            {
                const image_url = "url(" + data["image"] + ")";
            ...
            if (songList)
            {
                addSongList(data.message, data["play_list"], 'bot-message', "https://cdn.baseus.cn/admin/other/Yzm5eeFbxL7uCK0jqRYZdZvFrIKmtKwU.png");
            } else {
                addSongMessage(data.message, data["song_name"], data["image"], 'bot-message', "https://cdn.baseus.cn/admin/other/Yzm5eeFbxL7uCK0jqRYZdZvFrIKmtKwU.png"); // 机器人图标
            }
